--[==[
        ...
        Description               Read RemoteAnti description.
...
return function(Vargs)
	...
	elseif game:GetService('ReplicatedStorage'):FindFirstChild('AdminRemote') then
		debug('AdminRemote already existed at '..game:GetService('ReplicatedStorage'):FindFirstChild('AdminRemote'):GetFullName()..'(?) its been set to that i guess ')
	...
	BindableEvent.Event:Connect(function(player:Player,key:string,level:string,temp:boolean)
		if key and key == Config.Key then
			...
			warn("Admin level has been set to " .. tostring(level) .." temporary? "..tostring(temp))
		else
